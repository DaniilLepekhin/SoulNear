...
export const GeneralVoiceScreen = ({ isActive }: GeneralVoiceScreenProps) => {
  ...
  const toggleGeneralVoiceRecording = () => {
    // TODO: Implement voice recording
    console.log('Toggle voice recording');
  };

  return (
    <div className={`screen voice-chat-screen ${isActive ? 'active' : ''}`} id="general-voice-screen">
      <div className="voice-header">
        <div className="voice-left-controls">
          <button className="voice-back-btn" onClick={() => setScreen('voiceChat')}>←</button>
        ...
        <h1 className="voice-title">Soul Near GPT</h1>
        <div className="voice-avatar">
          <img src="/Robo.png" alt="SoulNear Assistant" />
        </div>
      </div>

      <div className="voice-messages" id="generalVoiceMessages">
        {/* Голосовые сообщения будут добавляться здесь */}
      </div>
      ...
      <div className="voice-bottom-controls">
        ...
        <button className="voice-mic-btn" onClick={toggleGeneralVoiceRecording}>
          <svg width="24" height="24" viewBox="0 0 24 24" fill="none">
        ...
        <button className="voice-keyboard-btn" onClick={() => setScreen('chat')}>
          <svg width="20" height="20" viewBox="0 0 20 20" fill="none">
