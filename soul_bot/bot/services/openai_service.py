...
⚠️ НЕ КОПИРУЙ эти фразы и структуры. Найди НОВЫЙ способ сказать то же самое. Избегай повторов типа "Это довольно распространенное...", "Важно помнить...", "Может быть, стоит...".""".format(
    ...
    # 🆕 Адаптация длины ответа по последнему сообщению
    ...
    if realtime_reference:
        ...
        if msg_len < 20:  # Короткое сообщение ("угу", "да", "хорошо")
            sections.append("""
## ⚡ Быстрый ориентир
Пользователь ответил очень кратко ({} символов). Держи ответ в 1–2 предложениях, без длинных рассуждений. Он может быть уставшим или занятым — не перегружай.""".format(msg_len))
        elif msg_len > 200:  # Длинное сообщение (развёрнутое)
            sections.append("""
## 📝 Ориентир на глубину
Пользователь написал развёрнуто ({} символов). Он готов к глубокому разговору. Ответь подробно (3–5 абзацев), раскрой тему и задай уточняющие вопросы.""".format(msg_len))
